...
const stream = ( socket ) => {
    ...
    //         socket.emit( 'initial', {room:data.room,socketId:data.socketId,socketIdAdmin:rooms[roomFI].host,
    //         isAllow : rooms[roomFI].participants.find((el)=>{return el == data.socketId;}) ? true : false } );
    //     }
    // }

    socket.on( 'allow-user', ( data ) => {  
        console.log("allow-user",data.socketIdAdmin+"-host",data);
        socket.to( data.socketIdAdmin+"-host" ).emit( 'allow-user', {...data } );
    }); 
